...
class JobApplicationExtensionManager {
  ...
  async initializeAIProvider() {
    try {
      ...
      if (aiConfig.apiKey && aiConfig.provider) {
        this.aiProvider = AIProviderFactory.create(aiConfig.provider, aiConfig.apiKey);
    ...
    try {
      switch (request.action) {
        ...
        case 'generateCoverLetter':
          const coverLetter = await this.generateCoverLetterForJob(request.jobInfo);
          sendResponse({ success: true, data: coverLetter });
          break;

        case 'generateInterviewQuestions':
          const questions = await this.generateInterviewQuestions(request.jobTitle);
          sendResponse({ success: true, data: questions });
          break;

        case 'saveApplication':
          const application = await this.applicationService.createApplication(request.jobInfo, request.url);
        ...
        case 'updateApplication':
          const updatedApp = await this.applicationService.updateApplication(request.id, request.updates);
  ...
  fillFormOnPage(userData, fieldMappings) {
    try {
      ...
      const detectedFields = fieldDetection.detectFieldsWithScoring(fieldMappings);
      
      let filledCount = 0;

      // Fill personal information
      if (detectedFields.firstName && userData.personalInfo.firstName) {
        FormFillingUtils.fillField(detectedFields.firstName, userData.personalInfo.firstName);
  ...
  async generateCoverLetterForJob(jobInfo) {
    ...
    try {
      const userData = await this.dataService.getUserData();
      const coverLetter = await this.aiProvider.generateCoverLetter(jobInfo, userData);
      
      // Save to application if exists
      if (jobInfo.applicationId) {
        await this.applicationService.addCoverLetter(jobInfo.applicationId, coverLetter);
      }
  ...
  async generateInterviewQuestions(jobTitle) {
    ...
    try {
      const userData = await this.dataService.getUserData();
      const questions = await this.aiProvider.generateInterviewQuestions(jobTitle, userData);
